...
```
I already installed nycflights13, so I only had to load it after loading tidyverse.
...
print(plot2)
```
I initially tried using the `!(is.na(air_time))` but it still said that there were five rows with missing data, but using the air_time >= 0 gave no warnings.
...
print(plot3)
```
This time I used the `na.rm=TRUE` to block any error messages about blank values.
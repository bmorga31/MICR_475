...
The only difference between the groups is the y values, and the distribution of the y values. Everything else is the same.
...
This data has some small differences between datasets for the mean and standard deviations of the x and y variables, but the differences are incredibly small.
...
`left_join()` joins two data frames, but keeps all rows present in the "left" data frame, and omits rows from the right 

`right_join()` joins two data frames, but keeps all rows present in the "right" data frame, and omits rows from the left side

`inner_join()` joins two data frames, keeping all rows from each data frame
...
print(flights_plot)
```
Delays appear to increase as wind speed increases, but delays are not dependent solely on wind speeds because there are still delays at low winds speeds.